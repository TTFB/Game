	...
	uiLayer(this, 0,0,0,0)
{
	...
	MeshEntity * cube = new MeshEntity(MeshFactory::getCubeMesh(4.0f));
	cube->setShader(baseShader, true);
	childTransform->addChild(cube);
	cube->parents.at(0)->translate(0.f, 4.f, 10.f);

	MeshEntity * ground = new MeshEntity(MeshFactory::getPlaneMesh(40.0f));
	childTransform->addChild(ground);
	ground->parents.at(0)->rotate(-90.f, 1.f, 0.f, 0.f, kOBJECT);
	ground->setShader(baseShader, true);
